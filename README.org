...
I wanted something lightweight that makes me happy. So here's an agent harness and runtime for Rust. Spawn AI agents as Tokio actors, give them tools and a sandbox, let them talk to LLMs. Headless, programmable, no GUI -- just Rust. The whole thing runs off a single =Runtime::builder()= call.
