...
An agent harness and runtime for Rust. Think of it like what Flue does for TypeScript, but in Rust -- a framework for building headless, programmable AI agents that run as Tokio actors with built-in supervision, sessions, sandboxing, and tool use. The whole thing runs off a single =Runtime::builder()= call.
